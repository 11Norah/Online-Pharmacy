...
export class MyordersComponent implements OnInit { 
  ...
  getorders(){
    ...
    while(i !=myordersize) {
      ...
      .innerHTML +='<div style=" ">'+'<strong  style="color:rgb(1, 1, 1) ;   text-shadow: 0 0 3px rgb(249, 208, 24), 0 0 5px rgb(249, 208, 24); font-size: xx-large; display:flex;">'+"Order #"+counter+"</strong>"+"<br>"+'<p style=" font-size: x-large; font-weight:bold;">   Date : '+this.myorders[i].orderTime +'  (Eastern European Standard Time)';

        }
        (document.getElementById('orderscontent')as HTMLBodyElement).innerHTML +='<br>'+'<br>'+'<div style="width:300px;height:410px;background-color: white; padding:0px;border-radius: 15px;box-shadow: 0px 0px 15px 2px rgb(249, 208, 24);transition: 0.5s;margin-bottom: 70px;">'
        ...
        +this.myorders[i+j].imageUrl+"></div>"+this.myorders[i+j].productName +"<br>"+"<br>"+'<span style="color:white;  margin-left: 5px; background-color: rgb(113, 113, 113); border: 2px solid black ;padding: 10px; border-radius: 25px;">Quantity</span>'
        +" : "+  this.myorders[i+j].quantity
        +"<br>"+"<br>"+"<br>"+' <span style="color:white;  background-color: rgb(113, 113, 113); border: 2px solid black ;padding: 15px; margin: 5px;  border-radius: 25px;">Cost</span>'
